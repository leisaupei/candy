...
namespace Creeper.PostgreSql.XUnitTest
{
	public class SelectSpecial : BaseTest
	{
		...
		public void ReturnJTokenInTuple()
		{
			...
			_dbContext.ExecuteNonQuery("update class.grade set create_time = now() + @years", System.Data.CommandType.Text,
		...
		{
			var result = _dbContext.ExecuteScalar<Guid?>("select id from people where id <> @id", cmdParams: new[] { new NpgsqlParameter("id", null) });
